...
var sql = {
  ...
  addressTransactions: swig.compileFile('./lib/sql/addressTransactions.sql'),
  addressUnspents: swig.compileFile('./lib/sql/addressUnspents.sql'),
  blocks: swig.compileFile('./lib/sql/blocks.sql'),
  blocksLatest: swig.compileFile('./lib/sql/blocksLatest.sql'),
  transactionInputs: swig.compileFile('./lib/sql/transactionInputs.sql'),
  transactionOutputs: swig.compileFile('./lib/sql/transactionOutputs.sql'),
  transactions: swig.compileFile('./lib/sql/transactions.sql'),
  transactionsLatest: swig.compileFile('./lib/sql/transactionsLatest.sql')
}
...
Database.prototype.transactionsGet = function(txIds, callback) {
  ...
  async.parallel({
    "details": runQuery.bind(null, this.connString, query, txIds),
    "inputs": runQuery.bind(null, this.connString, queryInputs, txIds),
    "outputs": runQuery.bind(null, this.connString, queryOutputs, txIds)
    ...
    try {
      results.inputs.forEach(function(row) {
        var txId = row.txId
      ...
      results.outputs.forEach(function(row) {
        var txId = row.txId
        if (!(txId in seen)) throw txId + ' is weird'
